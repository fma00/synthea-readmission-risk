...
- Dependencies: `pyproject.toml` (package metadata, src-layout) + `requirements.in`/`requirements-dev.in` compiled to hash-pinned `requirements.txt`/`requirements-dev.txt` via `pip-compile --generate-hashes`. Install as three separate invocations: `pip install -e .` (editable, no hashes), `pip install --require-hashes -r requirements.txt` (runtime deps — `pyspark`, added in the Big Join slice), then `pip install --require-hashes -r requirements-dev.txt`. Also requires a JDK (17+) on `PATH` for PySpark's local mode.
