...
**Requirement carried into slice 2 (the Big Join feature-engineering design), found empirically 2026-09-17:** local Synthea generation was run with `exporter.years_of_history=0` (full patient history, no truncation) so slice 1 wouldn't pre-judge how much lookback the join needs. This has a real cost — an isolated comparison at the same population/seed showed full history is **~2.6x** the size of Synthea's own 10-year-default export (1.6GB vs 637MB at 1,000 patients), and even the 10-year default extrapolates to ~6.4GB at 10,000 patients, well past the "a few hundred MB" that was informally expected. Two decisions the slice 2 design doc MUST make explicitly, not by default:
1. **Bound the history lookback window** to whatever the readmission model's actual feature window is (e.g. prior 1-2 years of comorbidities/utilization, not a patient's full simulated lifetime) — then set `exporter.years_of_history` to match for slice 1 regeneration, rather than leaving it at "keep everything."
2. **Decide which Synthea CSV tables the join actually needs.** `claims.csv`/`claims_transactions.csv` are billing-line-item exports, not clinical data, and are two of the three largest tables by volume (see the size comparison above) — their clinical relevance to a readmission-risk feature set should be argued for, not assumed by including everything `EXPECTED_CSV_TABLES` in `src/readmission_risk/pipeline/generation.py` currently validates for presence.
