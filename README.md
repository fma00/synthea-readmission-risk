...
Predicts **unplanned** 30-day hospital readmission from [Synthea](https://synthea.mitre.org/)-generated synthetic patient data. A PySpark "Big Join" builds a per-encounter feature table, a leakage-controlled pipeline trains a logistic regression baseline and a calibrated XGBoost challenger, both tracked in MLflow, and a Top-N triage CLI ranks held-out discharges. On the development-set test window (not a virgin holdout) logistic regression reaches ROC-AUC 0.869, about the same as a reason-code-only baseline (a per-admission-reason rate table fitted on the training rows, no patient features; 0.873), so the models add little on this small, synthetic sample.
