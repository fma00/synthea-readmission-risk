...
- New errors introduced by the diff are blockers; pre-existing errors are out of scope. If unsure whether an error is new, baseline against `main`:
  ...
  ruff check . > /tmp/baseline-lint.txt 2>&1
  ```
...
No separate typecheck step is configured yet (no mypy/pyright in use) — skip this block until the project adopts one.
...
No E2E tier exists for this project (backend/data pipeline, no served UI) — skip this block.

No codegen step is required for this project — skip this block.
...
- Test coverage gaps: code paths not covered by unit / integration tests. For bug fixes specifically: is there a regression test that fails before the fix and passes after?
- Data-science / pandas gotchas: train/test leakage (fitting scalers/encoders before splitting, or on the full dataset), unseeded randomness affecting reproducibility, `SettingWithCopyWarning` / chained-indexing bugs, in-place DataFrame mutation surprising callers, silent dtype coercion (e.g. int columns becoming float after a merge with NaNs), off-by-one errors in date/window calculations (readmission windows, lookback periods), class-imbalance handling, mismatched train/eval feature sets, model/artifact versioning drift
- Project-rule violations from CLAUDE.md (cite the relevant section)
- Dead code, leftover print/breakpoint() calls, stale comments referencing removed code
