...
> **Superseded by slice 2b (2026-09-20).** The all-cause label defined in the bullet below, and its "planned-readmission exclusion is explicitly NOT implemented" statement, were replaced by an unplanned-readmission label (planned = chemotherapy/radiation procedure; overlapping stays are one episode; only episode-terminal stays are index rows) and the gold table now ships a `_gold_metadata.json`. See [readmission-label-planned-exclusion-2026-09-20.md](readmission-label-planned-exclusion-2026-09-20.md). The text below is kept unchanged as history.
...
- Excluding planned readmissions from the label (full CMS HRRP fidelity) — this slice implements a simpler all-cause 30-day flag (see Scope > In). **[Done in slice 2b, 2026-09-20 — see readmission-label-planned-exclusion-2026-09-20.md.]**
