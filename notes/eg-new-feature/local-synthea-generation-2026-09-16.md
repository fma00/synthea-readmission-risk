...
## Post-implementation addendum (2026-09-17)

After implementation and the sanity-check runs, the full-run output (`exporter.years_of_history=0`, 10,000 patients) came in at **17GB**, well above informal expectations. An isolated comparison (same population/seed, Synthea's own 10-year default vs. this slice's full-history setting) confirmed full history costs **~2.6x** in size, but that alone doesn't explain the whole gap — even the 10-year default extrapolates to ~6.4GB at 10,000 patients. This is now carried forward as an explicit requirement for slice 2 (see CLAUDE.md's Architecture section, "Requirement carried into slice 2"): the join's design doc must deliberately choose a bounded history lookback window and decide which CSV tables it actually needs (`claims.csv`/`claims_transactions.csv` are billing artifacts and two of the three largest tables by volume) — not inherit this slice's "keep everything" default.
