...
Training: `python scripts/train_models.py --gold-dir data/gold/full_run_2b --reference-date 20260916 --test-start-date 20230101 --experiment-name readmission-risk-2b` (~8 s; MLflow experiment `readmission-risk-2b`; the first pair of runs was tagged `git_dirty=dirty` because the working tree was uncommitted, so both experiments were deleted and re-run on commit `de98db5` (pre-publication history rewrite; previously `d7d3908`; local MLflow run tags created before the rewrite still carry the old id): all four runs are tagged `git_dirty=clean`, and every metric, interval and the gold fingerprint reproduced exactly). Split summary as pinned above; the "only 41 positive rows in the test window (< 50)" warning fired, as expected.
